...
async function muatDaftarPemilik() {
  ...
  if (!window.supabaseClient) {
    body.innerHTML = '<tr><td colspan="6" class="text-center" style="padding:40px; color:var(--rose-600);">Koneksi database cloud tidak tersedia.</td></tr>';
    return;
  }
  ...
  try {
    ...
    body.innerHTML = '<tr><td colspan="6" class="text-center" style="padding:40px; color:var(--rose-600);">Gagal memuat data: ' + err.message + '</td></tr>';
  }
}
...
function renderRowsPemilik(data) {
  ...
  if (data.length === 0) {
    body.innerHTML = '<tr><td colspan="6" class="text-center" style="padding:40px; color:var(--slate-400);">Tidak ada pemilik toko yang ditemukan.</td></tr>';
    return;
  }
  ...
  body.innerHTML = data.map(function(u) {
    ...
    let deleteBtn = u.role !== 'superadmin' 
...
function filterPemilik() {
  ...
  let filtered = listPemilik.filter(function(u) {
    return (u.nama || '').toLowerCase().includes(query) || 
           ...
           (u.bisnis || '').toLowerCase().includes(query);
  });
...
function bukaModalPemilik(userId) {
  ...
  let target = listPemilik.find(function(u) { return u.user_id === userId; }) || {};
